...
class FormatWithColors(logging.Formatter):
    """
    A subclass of `logging.Formatter`, `FormatWithColors` adds color coding to log messages 
    based on their severity level for a more visually distinctive logging output, especially 
    useful in console or terminal environments.

    Class Attributes:
    - COLOR_MAP: A dictionary mapping log levels to ANSI color codes. It defines specific 
      colors for various log levels such as DEBUG, INFO, WARNING, ERROR, and CRITICAL.

    Constructor:
    - __init__(self, record_format)
        Initializes a new instance of the `FormatWithColors` class.
        Parameters:
            record_format (str): The format string for log records.
        This method sets up different formatters for each log level, applying the corresponding 
        color codes.

    Methods:
    - no_colors(self, flag)
        Disables or enables the use of colors in formatting.
        Parameters:
            flag (bool): If True, disables colors; if False, enables colors.

    - _formatter(self, level)
        An internal method to select the appropriate formatter based on the log level.
        Parameters:
            level (int): The log level.
        Returns: A `logging.Formatter` object corresponding to the given log level.

    - format(self, record)
        Overrides the `format` method of `logging.Formatter`.
        Formats the log record with the appropriate color based on its level.
        Parameters:
            record (logging.LogRecord): The log record to format.
        Returns: The formatted log record as a string.

    Usage Example:
    Creating and using an instance of FormatWithColors with a logger to output colored log messages.
    """
    COLOR_MAP = {
...
def k8s_token(context, namespace, secret_name):
    logger.debug(f'Retrieving token from secret {secret_name} in context {context} for namespace {namespace}')
    encoded_token = execute_os_command(f'kubectl'
                                       f' --context {context}'
                                       f' -n {namespace} get secret {secret_name}'
                                       f' -o jsonpath=\'{{.data.token}}\'')
    assert encoded_token, f'Failed to retrieve token from secret {secret_name} in context {context} for namespace {namespace}; run `v8 setup` and try again'
